...
本快速入门指南将向您展示如何下载 Cargo Admin 客户端 SDK，连接到 Cargo 管理服务，并提供一个简单文件上传器的示例。
...
import (
    ...
    "github.com/byteark-project/madmin-go"
)
...
func main() {
    ...
    // 初始化
    mdmClnt, err := madmin.New("your-cargo.example.com:9000", "YOUR-ACCESSKEYID", "YOUR-SECRETKEY", ssl)
    if err != nil {
        fmt.Println(err)
        return
    }
...
|:------------------------------------|:-----------------------------------------|:-------------------|:--------------------------|
| [`ServiceTrace`](#servicetracectx-contextcontext-alltrace-bool-donech-lt-chan-struct-lt-chan-traceinfo)     | [`ServerInfo`](#serverinfoctx-contextcontext-serverinfo-error)              | [`Heal`](#healctx-contextcontext-bucket-prefix-string-healopts-healopts-clienttoken-string-forcestart-bool-forcestop-bool-start-healstartsuccess-status-healtaskstatus-err-error)    | [`GetConfig`](#getconfigctx-contextcontext-byte-error) |
| [`ServiceStop`](#servicestopctx-contextcontext-error)       | [`StorageInfo`](#storageinfoctx-contextcontext-storageinfo-error)            |                    | [`SetConfig`](#setconfigctx-contextcontext-config-ioreader-error) |
| [`ServiceRestart`](#servicerestartctx-contextcontext-error) | [`AccountInfo`](#accountinfoctx-contextcontext-accountinfo-error)  |                    |                           |



| Top operations          | IAM operations                        | Misc                                              | KMS                             |
|:------------------------|:--------------------------------------|:--------------------------------------------------|:--------------------------------|
| [`TopLocks`](#=toplocksctx-contextcontext-lockentries-error) | [`AddUser`](#adduserctx-contextcontext-user-string-secret-string-error)                 | [`StartProfiling`](#startprofilingctx-contextcontext-profiler-string-error)               | [`GetKeyStatus`](#getkeystatusctx-contextcontext-keyid-string-kmskeystatus-error) |
|                         | [`SetPolicy`](#setpolicyctx-contextcontext-policyname-entityname-string-isgroup-bool-error)             | [`DownloadProfilingData`](#downloadprofilingdatactx-contextcontext-byte-error) |                                 |
|                         | [`ListUsers`](#listusersctx-contextcontext-mapstringuserinfo-error)             | [`ServerUpdate`](#serverupdatectx-contextcontext-updateurl-string-serverupdatestatus-error)                   |                                 |
|                         | [`AddCannedPolicy`](#addcannedpolicyctx-contextcontext-policyname-string-policy-iampolicypolicy-error) |                                                   |                                 |
...
|:------------------|:---------|:----------------------------------------------------------|
| `endpoint`        | _string_ | Cargo endpoint.                                           |
...
Sends a service action restart command to Cargo server.
...
Sends a service action stop command to Cargo server.
...
Enable HTTP request tracing on all nodes in a Cargo cluster
...
Get current `config.json` of a Cargo server.
...
Set a new `config.json` for a Cargo server.
...
Get the oldest locks from Cargo server.
...
Create a new canned policy on Cargo server.
...
Add a new user on a Cargo server.
...
Enable a canned policy `get-only` for a given user or group on Cargo server.
...
Lists all users on Cargo server.
...
Sends a update command to Cargo server, to update Cargo server to latest release. In distributed setup it updates all servers atomically.
...
from a Cargo server. The keyID is optional and the server will
...
``` go
    keyInfo, err := madmClnt.GetKeyStatus(context.Background(), "my-cargo-key")
    if err != nil {
       log.Fatalln(err)
    }
...
## License
This SDK is distributed under the [Apache License, Version 2.0](http://www.apache.org/licenses/LICENSE-2.0), see [LICENSE](https://github.com/byteark-project/madmin-go/blob/master/LICENSE).
